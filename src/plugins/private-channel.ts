...
import { IBotPlugin, IBot, Jsonable } from '../resources';
import { GuildMember, CategoryChannel, TextChannel, VoiceChannel, Message, Guild, Client, VoiceState, GuildChannel, PermissionOverwrites } from 'discord.js';
...
class PrivateChannel extends Jsonable {
    ...
    constructor(client: Client, owner: GuildMember) {
        ...
        owner.guild.channels.create('PRIVATE - ' + owner.displayName, {
    ...
    public addMember(member: GuildMember) {
        this.category.updateOverwrite(member, {
            VIEW_CHANNEL: true
